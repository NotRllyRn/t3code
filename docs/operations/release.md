...
- Builds a self-contained CLI archive per platform (`t3-<version>-<platform>-<arch>.tar.gz`, `.zip` on Windows) on the same runners as the desktop artifacts and attaches them to the GitHub Release with a `SHA256SUMS` file, on every channel, for all six targets: macOS arm64 and x64, Linux x64 and arm64, Windows x64 and arm64. Linux archives are built on runners of their own architecture because node-pty compiles from source there; the others are built on the platform's runner, cross-injecting the bundle into the target's Node where the runner's architecture differs (the same way the macOS x64 desktop app has always been built on the arm64 runner). Each archive is smoke-tested where the runner can execute it, which today excludes only Windows arm64.
...
4. Create release tag `vX.Y.Z` and push; workflow will:
   - build and smoke-test the six CLI archives
